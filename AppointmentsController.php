...
// dd($relations);
    ...
    public function endAppointment($id)
    {
        ...
        $employeeData=Appointment::where('employee_id', $id)->whereTime('finish_time', '>=', date("H:i", strtotime(now())))->get();
    
        // dd(count($employeeData));
        if(count($employeeData)==0){
            return back()->with('error',"appointment already ended");
        }

        $employee=Appointment::find($employeeData[0]->id);
        $employee->finish_time=now();
        $employee->save();
        return redirect()->route('admin.appointments.index')->with("success","Appointment Ended Successfully");
